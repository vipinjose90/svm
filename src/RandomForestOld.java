...
public class RandomForestOld {
    ...
    String prediction[];
    int featureSet[][] = new int[N][k];
    ID3 [] dt = new ID3 [N];
    float [][] treeTrainPred;
    float [][] treeTestPred;
    RandomForestSVM rf=new RandomForestSVM();
    
    RandomForestOld() throws FileNotFoundException {        
    ...
    void createFeatureVector(){
        ...
        for (int i=0; i<N; i++) {
            for(int j=0; j<k; j++){
                int ranPos = rnum.nextInt(trainCol);
                featureSet[i][j]=ranPos;
            }
        }
    }
    ...
    void createTrainPredict(){
        for(int i=0;i<N;i++){
            String [][] sData = new String [trainRow][k+1];
            for(int j=0;j<trainRow;j++){                                
                for(int l=0;l<k;l++){
                    sData[j][l]=String.valueOf(train[j][featureSet[i][l]]);
                }
                sData[j][k]=String.valueOf(trainLabels[j]);
            }
...
//        }  
    ...
    void createTestPredict(){
        for(int i=0;i<N;i++){
            String [][] sData = new String [testRow][k+1];
            for(int j=0;j<testRow;j++){                                
                for(int l=0;l<k;l++){
                    sData[j][l]=String.valueOf(test[j][featureSet[i][l]]);
                }
                sData[j][k]=String.valueOf(testLabels[j]);
            }
            dt[i].setTestData(sData);
